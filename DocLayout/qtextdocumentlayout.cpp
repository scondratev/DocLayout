...
public:
    QTextDocumentLayoutPrivate(QTextDocumentLayout* doc);
...
QTextDocumentLayoutPrivate::QTextDocumentLayoutPrivate(QTextDocumentLayout *doc)
    : fixedColumnWidth(-1),
      cursorWidth(1),
...
{
    QTextFrame *rootFrame = q_func()->document()->rootFrame();
...
//    const int blockPos = map.position(block);
    ...
    const int begin = q_func()->document()->findBlock(rootFrame->firstPosition()).fragmentIndex();
    const int end = q_func()->document()->findBlock(rootFrame->lastPosition()+1).fragmentIndex();

    QTextBlock b = q_func()->document()->findBlock(position);
    const int block = b.fragmentIndex();
    const int blockPos = b.position();


    QTextFrame::iterator it(rootFrame, block, begin, end);

    QTextFrame *containingFrame = q_func()->document()->frameAt(blockPos);
...
QFixed QTextDocumentLayoutPrivate::blockIndent(const QTextBlockFormat &blockFormat) const
{
    ...
    QTextObject *object = q_func()->document()->objectForFormat(blockFormat);
    if (object)
    ...
    qreal scale = 1;
    if (q_func()->paintDevice()) {
        scale = qreal(q_func()->paintDevice()->logicalDpiY()) / qreal(qt_defaultDpi());
    }

    return QFixed::fromReal(indent * scale * q_func()->document()->indentWidth());
}
...
{
    BorderPaginator paginator(q_func()->document(), rect, topMargin, bottomMargin, border);
...
//     qCDebug(lcDraw) << "drawFrame" << frame->firstPosition() << "--" << frame->lastPosition() << "at" << offset;
    ...
    if (table) {
        ...
        QFixed pageHeight = QFixed::fromReal(q_func()->document()->pageSize().height());
        if (pageHeight <= 0)
        ...
        if (frame == q_func()->document()->rootFrame())
...
//    painter->setPen(Qt::NoPen);
//    painter->setBrush(brush);
//    painter->drawRect(QRectF(p1.x(), p1.y(), p2.x() + wh, p2.y() + wh));
//    painter->restore();
}
#endif
                                                     ...
                                                     const QTextTableCell &cell) const
{
    ...
    BorderPaginator paginator(q_func()->document(), cellRect, topMarginAfterPageBreak, bottomMargin, 0);
    ...
    for (int page = paginator.topPage; page <= paginator.bottomPage; ++page) {
        ...
        const bool previousRowOnPreviousPage = !isFirstRow
                && !isHeaderRow
                && BorderPaginator(q_func()->document(),
                                               ...
                                               QTextBlock *cursorBlockNeedingRepaint, QPointF *cursorBlockOffset) const
{
    ...
    if (bg.style() != Qt::NoBrush) {
        const qreal pageHeight = q_func()->document()->pageSize().height();
                                          ...
                                          QTextFrame::Iterator it, const QList<QTextFrame *> &floats, QTextBlock *cursorBlockNeedingRepaint) const
{
    ...
    for (int i = 0; i < floats.count(); ++i) {
        ...
        if (handler) {
            ...
            handler->drawObject(painter, rect, q_func()->document(), pos, format);
        }
    }
}
...
//    qCDebug(lcDraw) << "drawBlock" << bl.position() << "at" << offset << "br" << tl->boundingRect();
    ...
    if (bg != Qt::NoBrush) {
        ...
        if (inRootFrame && q_func()->document()->pageSize().width() <= 0) {
            const QTextFrameData *fd = data(q_func()->document()->rootFrame());
    ...
    QTextObject *object = q_func()->document()->objectForFormat(bl.blockFormat());
                                              ...
                                              const QTextBlock &bl, const QTextCharFormat *selectionFormat) const
{
    ...
    QTextObject * const object = q_func()->document()->objectForFormat(blockFormat);
...
//    qCDebug(lcTable) << "layoutCell";
    ...
    layoutStruct.pageHeight = QFixed::fromReal(q_func()->document()->pageSize().height());
...
//    qCDebug(lcTable) << "layoutTable from" << layoutFrom << "to" << layoutTo << "parentY" << parentY;
    ...
    // cellspacing.
    td->borderCollapse =  true; //fmt.borderCollapse();
    ...
    const QTextOption::WrapMode oldDefaultWrapMode = q_func()->document()->defaultTextOption().wrapMode();
...
//                                 << "%, allocated width" << td->widths[i] << "remaining width now" << remainingWidth;
    ...
    // mode
    if (q_func()->document()->defaultTextOption().wrapMode() == QTextOption::WrapAtWordBoundaryOrAnywhere
        && contentsWidth > td->contentsWidth) {
        QTextOption to = q_func()->document()->defaultTextOption(); to.setWrapMode(QTextOption::WrapAnywhere);
        q_func()->_lock = true;
        q_func()->document()->setDefaultTextOption(to);
        q_func()->_lock = false;
    ...
    QTextOption to = q_func()->document()->defaultTextOption(); to.setWrapMode(oldDefaultWrapMode);
    q_func()->_lock = true;
    q_func()->document()->setDefaultTextOption(to);
    q_func()->_lock = false;
...
//    qCDebug(lcLayout, "layoutFrame (%d--%d), parent=%p", f->firstPosition(), f->lastPosition(), f->parentFrame());
    ...
    bool fullLayout = (f == q_func()->document()->rootFrame() && !fd->fullLayoutCompleted);
    {
        QTextFrameFormat fformat = f->frameFormat();
...
//    qCDebug(lcLayout) << "layoutFlow from=" << layoutFrom << "to=" << layoutTo;
    ...
    const bool inRootFrame = (it.parentFrame() == q_func()->document()->rootFrame());
    if (inRootFrame) {
    ...
    while (!it.atEnd()) {
        ...
        if (c) {
            ...
            if (fformat.position() == QTextFrameFormat::InFlow) {
                ...
                if (q_func()->document()->pageSize().height() > 0.0f)
                    cd->sizeDirty = true;
    ...
    if (inRootFrame) {
        ...
        if (it.atEnd()) {
            ...
            QCheckPoint cp;
            cp.y = layoutStruct->y;
            cp.positionInFrame = q_func()->document()->characterCount();
            cp.minimumWidth = layoutStruct->minimumWidth;
            cp.maximumWidth = layoutStruct->maximumWidth;
            cp.contentsWidth = layoutStruct->contentsWidth;
            checkPoints.append(cp);
...
//                      << "; width" << layoutStruct->x_right - layoutStruct->x_left << "(maxWidth is btw" << tl->maximumWidth() << ')';
    ...
    QFixed extraMargin;
    if (q_func()->document()->defaultTextOption().flags() & QTextOption::AddSpaceForLineAndParagraphSeparators) {
...
//        qCDebug(lcLayout) << "do layout";
        QTextOption option = q_func()->document()->defaultTextOption();
        option.setTextDirection(dir);
        option.setTabs( blockFormat.tabPositions() );

        Qt::Alignment align = q_func()->document()->defaultTextOption().alignment();
        if (blockFormat.hasProperty(QTextFormat::BlockAlignment))
            align = blockFormat.alignment();
        option.setAlignment(align); // for paragraph that are RTL, alignment is auto-reversed;

        if (blockFormat.nonBreakableLines() || q_func()->document()->pageSize().width() < 0) {
...
{
    p_ptr = new QTextDocumentLayoutPrivate(this);
    _lock = false;
...
void QTextDocumentLayout::draw(QPainter *painter, const PaintContext &context)
{
    ...
    if (document()->pageSize().width() == 0 && d->viewportRect.isValid()) {
...
void QTextDocumentLayout::documentChanged(int from, int oldLength, int length)
{
    ...
    const int documentLength = document()->characterCount(); //->length();
    ...
    if (fullLayout) {
        ...
        d->checkPoints.clear();
        data(document()->rootFrame())->fullLayoutCompleted = false;
        d->layoutStep();
    } else {
...
void QTextDocumentLayout::resizeInlineObject(QTextInlineObject item, int posInDocument, const QTextFormat &format)
{
    ...
    Q_ASSERT(f.isValid());
    QTextObjectInterface* handler = handlerForObject(f.objectType());
    if (!handler)
        return;

    QSizeF intrinsic = handler->intrinsicSize(document(), posInDocument, format);
...
int QTextDocumentLayout::layoutStatus() const
{
    ...
    if (pos == -1)
        return 100;
    return pos * 100 / document()->characterCount();
}
...
qreal QTextDocumentLayoutPrivate::scaleToDevice(qreal value) const
{
    if (!q_func()->paintDevice())
        return value;
    return value * q_func()->paintDevice()->logicalDpiY() / qreal(qt_defaultDpi());
}
...
QFixed QTextDocumentLayoutPrivate::scaleToDevice(QFixed value) const
{
    if (!q_func()->paintDevice())
        return value;
    return value * QFixed(q_func()->paintDevice()->logicalDpiY()) / QFixed(qt_defaultDpi());
}
